...
mesh = Collision(100, trimesh.primitives.Sphere(0.05), method)
T = np.eye(4)
T[:3, :3] = Rotation.from_rotvec([0.0, 1.0, 0.0]).as_matrix()
T[:3, 3] = np.array([0.0, 0.0, 1.0])
mesh.apply_transform(T)

t0 = time.time()
distances = robot_collision_detector.collision_detection(q, [mesh])
t1 = time.time()

print("碰撞检测耗时：", t1 - t0)
print("最小距离：", np.min(distances))
print("碰撞检测信息：", distances)

robot_collision_detector.show_collision(q, [mesh])
