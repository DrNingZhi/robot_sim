...
if ~run_opt
    ...
    p_discrete=[0,0,0];    %设计变量的离散程度，0表示是连续量
    size_pop=50;  %种群数量
    Ini_gene_method=0;  %采用第一种初始化方法，随机产生初值
    p0_ini_para=0;  %第一种方法无需这个参数
    Conv_method=0;  %判断收敛的方式，0表示以目标函数变化为准
    Obj_conv=0.001; %如果目标函数值的变化小于0.001，认为收敛结束计算
    pop = Complex_optimization_modify(p_range,p_discrete,size_pop,Ini_gene_method,p0_ini_para,Conv_method,Obj_conv);
end
