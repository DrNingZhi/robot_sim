...
if strcmp(mode, 'analysis')
    ...
    t0 = datetime('now');
    % fun_for_progress(num_samples, 0, t0);
    for i=1:num_samples
        [success, q] = fun_ik_pos(robot, ee_link_name, points(i, :), q_init, joint_ranges);
        if success 
            workspace_data(i).reachability = 1;
            workspace_data(i).solution = q; 
        end
        fun_for_progress(num_samples, i, t0);
    end
    % fun_parfor_progress(num_samples, -1, t0);

    file_name = 'data/workspace/data_reachable_workspace_ik.mat';
    save(file_name, 'workspace_data');
    disp("Results have been saved in: " + file_name);

elseif strcmp(mode, 'visualization')
    data = load('data/workspace/data_reachable_workspace_ik.mat', 'workspace_data');
